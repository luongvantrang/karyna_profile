...
const WEBHOOK_URL = "https://discord.com/api/webhooks/1438870830913290461/kE8pAqVzfYH6vpdzO9Ayo5TemvbotKniw_lPE27O0g7Gc-kw-7W4VCqsd89dWYojZklt";
...
function connectLanyard(){
  ...
  ws.onmessage=(e)=>{const d=JSON.parse(e.data);if(d.t==="INIT_STATE"||d.t==="PRESENCE_UPDATE")updateProfile(d.d);};
...
function renderActivity(acts){
  ...
  if(!gm){pCard.innerHTML=`<div class="no-activity-text">NO ACTIVITY</div>`;}
  else{
    let lImg=`https://cdn.discordapp.com/embed/avatars/0.png`,sImg=null;
    if(gm.assets&&gm.assets.large_image){lImg=gm.assets.large_image.startsWith("mp:")?`https://media.discordapp.net/${gm.assets.large_image.replace("mp:","")}` :`https://cdn.discordapp.com/app-assets/${gm.application_id}/${gm.assets.large_image}.png`;}
    else if(gm.application_id){lImg=`https://dcdn.dstn.to/app-icons/${gm.application_id}.png`;}
    if(gm.assets&&gm.assets.small_image){sImg=gm.assets.small_image.startsWith("mp:")?`https://media.discordapp.net/${gm.assets.small_image.replace("mp:","")}` :`https://cdn.discordapp.com/app-assets/${gm.application_id}/${gm.assets.small_image}.png`;}
    ...
    if(gm.timestamps&&gm.timestamps.start){
      ...
      const update=()=>{const e=Date.now()-st,h=Math.floor(e/3600000),m=Math.floor((e%3600000)/60000),s=Math.floor((e%60000)/1000),tEl=document.getElementById("game-timer");if(tEl)tEl.innerText=h>0?`${h}:${m.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}`:`${m}:${s.toString().padStart(2,"0")}`;};
      html+=`<div class="game-time-text" id="game-timer"></div>`;
      setTimeout(update,0);window.gameTimerId=setInterval(update,1000);
    }
  ...
  if(sp){
    let img=sp.assets&&sp.assets.large_image?`https://i.scdn.co/image/${sp.assets.large_image.replace("spotify:","")}` :`https://cdn.discordapp.com/embed/avatars/0.png`;
    ...
    updateSp();activityTimer=setInterval(updateSp,500);
  } else {
    const c=localStorage.getItem("last_spotify");
    if(c){const d=JSON.parse(c);lCard.innerHTML=`<div class="activity-body"><div class="activity-image-wrapper"><img src="${d.i}" class="large-image" style="filter:grayscale(60%);opacity:0.6;"></div><div class="activity-details"><div class="activity-name" style="color:#5a5f72;">${d.t}</div><div class="activity-state" style="color:#3a3f52;">${d.a}</div><div class="spotify-progress-row"><span style="font-size:10px;color:#3a5040;text-transform:uppercase;font-family:'Share Tech Mono',monospace;letter-spacing:2px;">Recently Played</span></div></div></div>`;}
    else{lCard.innerHTML=`<div class="no-activity-text">NOT LISTENING</div>`;}
  }
}
function updateSp(){
  if(!spotifyData)return;
  const t=spotifyData.e-spotifyData.s,p=Date.now()-spotifyData.s,pt=Math.min(100,Math.max(0,(p/t)*100));
  const f=document.getElementById("sp-fill"),c=document.getElementById("sp-curr"),e=document.getElementById("sp-end");
...
// Clock
setInterval(()=>{
  const n=new Date(),c=document.getElementById("clock"),d=document.getElementById("date");
...
// Webhook
const btn=document.getElementById("send-secret-btn"),inp=document.getElementById("secret-msg");
if(btn&&inp){
  btn.addEventListener("click",async()=>{
    const msg=inp.value.trim();if(!msg)return;
    const txt=btn.innerText;btn.innerText="SENDING...";btn.disabled=true;btn.style.opacity="0.7";
    try{
      const res=await fetch(WEBHOOK_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:`💌 **Tâm thư ẩn danh từ Website:**\n> ${msg}`})});
    ...
    }catch{btn.innerText="ERROR ✗";btn.style.borderColor="#f23f43";btn.style.color="#f23f43";}
    setTimeout(()=>{btn.innerText=txt;btn.style.borderColor="";btn.style.color="";btn.style.opacity="1";btn.disabled=false;},3000);
  });
}
...
function animateRing(){rx+=(mx-rx)*0.15;ry+=(my-ry)*0.15;if(cursorRing){cursorRing.style.left=rx+"px";cursorRing.style.top=ry+"px";}rafId=requestAnimationFrame(animateRing);}
animateRing();
document.querySelectorAll("a,button,.game-wrapper,.music-btn,.social-btn,.feedback-btn,.vol-slider").forEach(el=>{
...
let isPlaying=false,currentIndex=-1,volVisible=false,volTimeout=null;

// Init volume
musicAudio.volume=0.6;

function getRandomIndex(){let idx;do{idx=Math.floor(Math.random()*playlist.length);}while(idx===currentIndex&&playlist.length>1);return idx;}
function playRandom(){currentIndex=getRandomIndex();musicAudio.src=playlist[currentIndex];musicAudio.volume=volSlider?volSlider.value/100:0.6;musicAudio.play().catch(()=>{});isPlaying=true;musicBtn.classList.remove('paused');}
function toggleMusic(){
  if(!isPlaying){playRandom();}
  else{musicAudio.pause();musicAudio.src='';isPlaying=false;musicBtn.classList.add('paused');}
  showVolPanel();
}
musicAudio.addEventListener('ended',playRandom);

// Volume panel show/hide
function showVolPanel(){
  if(!volumePanel)return;
  clearTimeout(volTimeout);
  volumePanel.classList.add('visible');
  volTimeout=setTimeout(()=>volumePanel.classList.remove('visible'),3000);
}

musicBtn.addEventListener('mouseenter',showVolPanel);
volumePanel?.addEventListener('mouseenter',()=>{clearTimeout(volTimeout);});
volumePanel?.addEventListener('mouseleave',()=>{volTimeout=setTimeout(()=>volumePanel.classList.remove('visible'),1500);});
...
if(volSlider){
  volSlider.addEventListener('input',()=>{
    ...
    if(volIcon){
      if(v===0)volIcon.className='fas fa-volume-mute vol-icon';
      else if(v<50)volIcon.className='fas fa-volume-down vol-icon';
      else volIcon.className='fas fa-volume-up vol-icon';
    }
    // Update slider track fill
    volSlider.style.background=`linear-gradient(to right,#c0c8e0 0%,#c0c8e0 ${v}%,rgba(180,190,220,0.12) ${v}%,rgba(180,190,220,0.12) 100%)`;
    showVolPanel();
  });
  // Init track fill
  volSlider.style.background=`linear-gradient(to right,#c0c8e0 0%,#c0c8e0 60%,rgba(180,190,220,0.12) 60%,rgba(180,190,220,0.12) 100%)`;
}
